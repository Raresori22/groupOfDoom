...
int buttonPin = 3, lightButton = 14, humidityButton = 16, bothButton = 15;
...
void loop() {
    ...
    if(updateFlag) { // Update Flag For When Interrupt Service is Called 
      ...
      lcd.clear();
      lcd.setCursor(0, 0); 
      lcd.print("Light: "); lcd.print(A4_average);
      lcd.setCursor(19, 0);
    ...
    send_MQTT_data(A4_average, weatherData);
...
void readButtons() {
    if(digitalRead(bothButton) == HIGH) {
        light = 1; humidity = 1;
    } else if(digitalRead(humidityButton) == HIGH) {
        humidity = 1; light = 0;
    } else if(digitalRead(lightButton) == HIGH) {
        humidity = 0; light = 1;
    }
}
...
void send_MQTT_message(int data, String invalidData) {
  ...
  if (light == 1 && humidity == 1) {
    ...
    sprintf(buffer, "IOTJS={\"S_name1\":\"GoD_light\",\"S_value1\":%d}", data);
        if (client.publish(outTopic, buffer)) {
      Serial.println("Light data sent:");
      Serial.println(buffer);
    } else {
      Serial.println("Light send failed!");
    }
    sprintf(buffer, "IOTJS={\"S_name1\":\"GoD_humidity\",\"S_value1\":-1}");
...
void send_MQTT_message(int data, double humidData) {
  ...
  sprintf(buffer, "IOTJS={\"S_name1\":\"GoD_humidity\",\"S_value1\":%s}", humStr);
    if (client.publish(outTopic, buffer)) {
